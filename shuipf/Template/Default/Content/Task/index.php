...
<div style="width: 720px; margin: 0 auto;">
	...
	<table class="table table-bordered table-hover table-condensed table-striped">
		<tbody>
			<tr>
				<td colspan="4"><strong>访问量数据:</strong></td>
			</tr>
			<tr>
				<td>访问人数</td>
				<td><?php echo $uv?>（人）</td>
	...
	<table class="table table-bordered table-hover table-condensed table-striped">
		<tbody>
			<tr>
				<td colspan="4"><strong>微信粉丝数据:</strong></td>
			</tr>
			<tr>
				<td>粉丝总人数</td>
				<td>关注人数</td>
	...
	<table class="table table-bordered table-hover table-condensed table-striped">
		<tbody>
			<tr>
				<td colspan="4"><strong>关键字相关数据:</strong></td>
			</tr>
...
.my_user_important td {
	...
	<table class="table table-bordered table-hover table-condensed table-striped">
		<tbody>
			...
			<tr>
				...
				<td>已付款数</td>
				<td>未付款数</td>
				<?php foreach ($dia['data'] as $k=>$d){?>
    				<td><?php echo $d['star']?>星级</td>
				<?php }?>
			</tr>
			<tr>
				<td><?php echo $dia['num']?>（个）</td>
				<td><?php echo $dia['pay']?>（个）</td>
				<td><?php echo $dia['not_pay']?>（个）</td>
				<?php foreach ($dia['data'] as $k=>$d){?>
    				<td><?php echo $d['num']?>（个）</td>
				<?php }?>
			</tr>
		</tbody>
	</table>	
	<table class="table table-bordered table-hover table-condensed table-striped my_user_important">
		<tbody>
			<tr>
				<td colspan="4"><strong>课程订单数据:</strong></td>
			</tr>
			<?php if(!empty($courseOrder)){?>
    			<?php foreach ($courseOrder as $k=>$c){?>
    			<tr>
    				<td><?php echo $c['status']?></td>
    				<td><?php echo $c['num']?>（笔）</td>
    			</tr>
    			<?php }?>
			<?php }else {?>
			    <tr><td colspan="4">无</td></tr>
	...
	<table class="table table-bordered table-hover table-condensed table-striped my_user_important">
		<tbody>
			<tr>
				<td colspan="4"><strong>铁杆社员订单数据:</strong></td>
			</tr>
			<?php if(!empty($vip)){?>
    			<?php foreach ($vip as $k=>$c){?>
    			<tr>
    				<td><?php echo $c['status']?></td>
    				<td><?php echo $c['num']?>（笔）</td>
    			</tr>
