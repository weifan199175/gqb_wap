...
class ManagementController extends AdminBase {
    ...
    public function manager() {
        ...
        $User = D('Admin/User')->alias('u')->join('jbr_fenbu as f ON f.id = u.fenbu_id',"left")->field("u.*,f.name")->where($where)->limit($page->firstRow . ',' . $page->listRows)->order(array('u.id' => 'DESC'))->select();
        foreach ($User as $k=>$u){
            if($u['fenbu_id'] == 0){//修改一个显示名称（让管理者不要产生误会）
                switch ($u['role_id']) {
                    case 5:
                        $User[$k]['name'] = "不相关";
                        break;
    ...
    public function edit() {
        ...
        if (IS_POST) {
            ...
            $this->assign("fenbu", M("fenbu")->field(array("id","name"))->select());
