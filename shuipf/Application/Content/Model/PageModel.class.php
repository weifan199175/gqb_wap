...
class PageModel extends Model {
    ...
    public function savePage($post) {
        ...
        if ($data) {
			...
			//M('test')->data(array('content' => 'info1' . serialize($info)))->add();
            if ($info) {
				M('test')->data(array('content' => 'info1' . $catid))->add();
                if ($this->where(array('catid' => $catid))->data($data)->save() !== false) {
					M('test')->data(array('content' => 'info2'))->add();
                    //更新附件状态，把相关附件和文章进行管理
                    service("Attachment")->api_update('', 'catid-' . $catid, 1);
                    return true;
                }
            } else {
				M('test')->data(array('content' => 'info3'))->add();
                if ($this->add($data) !== false) {
					M('test')->data(array('content' => 'info4'))->add();
                    //更新附件状态，把相关附件和文章进行管理
                    service("Attachment")->api_update('', 'catid-' . $catid, 1);
                    return true;
                }
