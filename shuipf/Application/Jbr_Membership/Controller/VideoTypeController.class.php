...
class VideoTypeController extends AdminBase{	
		
		public $linkapply=null;
		
		public function _initialize(){
		parent::_initialize ();
			$this->linkapply=M("videoType");
	}
		...
		*/
		public function add()
		{
			if(!empty($_POST)){			
				...
				$this->linkapply->typename=$_POST['typename'];
				$this->linkapply->remarks=$_POST['remarks'];
				
				$result = $this->linkapply->add();
				if($result){
					$this->success('添加成功',U('VideoType/index'));
				}else{
					 $this->error('添加失败');
				}
			}
		...
		*/
		public function edit()
		{
			...
			if(!empty($_POST))
			{
				...
				if($result !==false)
				{
					$this->success('修改成功',U('VideoType/index'));
				}
				else
				{
					 $this->error('修改失败');
