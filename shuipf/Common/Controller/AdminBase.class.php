...
class AdminBase extends ShuipFCMS {
    ...
    protected function _initialize() {
        ...
        //由于非超级管理员也要拥有修改会员备注信息的权限，所以在if判断后面又加了2个判断
        if (false == RBAC::AccessDecision(MODULE_NAME) && (CONTROLLER_NAME != "Membership" && ACTION_NAME != "saveExtra")) {
            //检查是否登录
        ...
        $this->competence();
        //后台token
        $token = D("wp_member_public")->where(array('is_use','1'))->select();
       	define('TOKEN', $token[0]['token']);
    }
